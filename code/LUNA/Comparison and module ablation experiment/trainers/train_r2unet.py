...
def train_r2unet():
    ...
    model = R2UNet(in_channels=1, out_channels=1).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))

    return train_model(model, train_dataset, val_dataset, test_dataset, "R2U-Net", "r2unet")