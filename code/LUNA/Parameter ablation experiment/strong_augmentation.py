...
# ========== 强增强 ==========
class StrongAugmentationPreprocessor:
    def __init__(self, target_size=(256, 256)):
        ...
        self.train_transform = A.Compose([
            ...
            # 0-360°随机旋转
            A.Rotate(limit=360, p=0.6, border_mode=0),
            # 增强的偏移、缩放、旋转
            A.ShiftScaleRotate(
                shift_limit=0.1,      # 从0.05增加到0.1
                scale_limit=0.2,      # 从0.1增加到0.2
                rotate_limit=20,      # 从10增加到20
                border_mode=0,
                p=0.7                 # 从0.5增加到0.7
            ),
            # 增强的亮度/对比度/噪声/模糊组合
            A.OneOf([
                A.RandomBrightnessContrast(
                    brightness_limit=0.2,  # 从0.1增加到0.2
                    contrast_limit=0.2,    # 从0.1增加到0.2
                    p=0.8
                ),
                A.GaussNoise(var_limit=(10.0, 20.0), p=0.6),  # 增强噪声
                A.MotionBlur(blur_limit=5, p=0.4),  # 增强模糊
                A.MedianBlur(blur_limit=3, p=0.3),
            ], p=0.8),  # 从0.5增加到0.8
            # 增强的粗dropout
            A.CoarseDropout(
                max_holes=12,        # 从8增加到12
                max_height=24,       # 从16增加到24
                max_width=24,        # 从16增加到24
                min_holes=2,         # 从1增加到2
                min_height=12,       # 从8增加到12
                min_width=12,        # 从8增加到12
                fill_value=0,
                p=0.5                # 从0.3增加到0.5
            ),
            # 额外的高斯噪声
            A.GaussNoise(var_limit=(10.0, 20.0), p=0.6),
            # 添加弹性变换
...
def train_model():
    ...
    preprocessor = StrongAugmentationPreprocessor(target_size=(256, 256))  # 使用强增强预处理器
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, os.path.join(BASE_PATH, "trained_models", "best_rca_unet_strong_aug.pth"))
    ...
    # 测试最佳模型
    test_model(model, test_loader, DEVICE, os.path.join(BASE_PATH, "trained_models", "best_rca_unet_strong_aug.pth"))
...
def plot_metrics(history):
    ...
    plt.tight_layout()
    plt.savefig("results/training_metrics_strong_augmentation.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Metrics plot saved to results/training_metrics_strong_augmentation.png")
