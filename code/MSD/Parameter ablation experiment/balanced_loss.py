        ...
        if self.is_training:
            ...
            if random.random() > 0.4:
                ...
                if scale > 1.0:
                    ...
                    image = np.pad(image_resized, ((top_pad, pad_h - top_pad), (left_pad, pad_w - left_pad)),
                                   mode='constant', constant_values=0)
                    mask = np.pad(mask_resized, ((top_pad, pad_h - top_pad), (left_pad, pad_w - left_pad)),
                                  mode='constant', constant_values=0)
...
class OptimizedCombinedLoss(nn.Module):
    def __init__(self, alpha=0.5, beta=0.5):  # 默认平衡权重
        super(OptimizedCombinedLoss, self).__init__()
        self.alpha = alpha  # Dice损失权重（0.5）
        self.beta = beta    # BCE损失权重（0.5）
        self.dice_loss = StableDiceLoss()
        self.bce_loss = nn.BCEWithLogitsLoss()
    def forward(self, inputs, targets):
        dice_loss = self.dice_loss(inputs, targets)
        bce_loss = self.bce_loss(inputs, targets)
        total_loss = self.alpha * dice_loss + self.beta * bce_loss  # 平衡权重计算
        return total_loss
...
def train_model():
    ...
    train_dataset = OptimizedLungDataset(os.path.join(DATA_DIR, "train"), os.path.join(DATA_DIR, "train"), is_training=True)
    val_dataset = OptimizedLungDataset(os.path.join(DATA_DIR, "val"), os.path.join(DATA_DIR, "val"), is_training=False)
    ...
    print(f"模型参数总数（平衡权重α=0.5，β=0.5）: {total_params:,}")  # 修改标识

    # 显式初始化平衡权重损失函数
    criterion = OptimizedCombinedLoss(alpha=0.5, beta=0.5)

    optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY, betas=(0.9, 0.999))
    plateau_scheduler = ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=15, min_lr=1e-7, verbose=True, cooldown=5)
    cosine_scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=20, T_mult=1, eta_min=1e-7)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # 进度条添加“平衡权重”
        train_progress = tqdm(train_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Train (平衡权重α=0.5，β=0.5)]")
        optimizer.zero_grad()
        ...
        for i, (images, masks) in enumerate(train_progress):
            ...
            if (i + 1) % accumulation_steps == 0:
                ...
                optimizer.zero_grad()
            with torch.no_grad():
                dice, sensitivity, specificity, iou, precision, hd95, asd = calculate_metrics(torch.sigmoid(outputs), masks)
            ...
            train_hd95 += hd95
            train_asd += asd
            train_progress.set_postfix({"Loss": f"{loss.item() * accumulation_steps:.4f}", "Dice": f"{dice:.4f}", "Precision": f"{precision:.4f}", "IoU": f"{iou:.4f}"})
        ...
        with torch.no_grad():
            # 进度条添加“平衡权重”
            val_progress = tqdm(val_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Val (平衡权重α=0.5，β=0.5)]")
            for images, masks in val_progress:
                ...
                val_hd95 += hd95
                val_asd += asd
                val_progress.set_postfix({"Loss": f"{loss.item():.4f}", "Dice": f"{dice:.4f}", "Precision": f"{precision:.4f}", "IoU": f"{iou:.4f}"})
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, "results/best_model_balanced_loss.pth")  # 平衡权重
            print(f"New best model (平衡权重α=0.5，β=0.5) saved with Dice: {val_dice:.4f}, IoU: {val_iou:.4f}")  # 修改日志
            patience_counter = 0
        else:
            patience_counter += 1

        if patience_counter >= max_patience:
            print(f"Early stopping triggered after {epoch + 1} epochs (平衡权重α=0.5，β=0.5)")  # 修改日志
        ...
        # 训练日志
        print(f"Epoch {epoch + 1}/{NUM_EPOCHS} (平衡权重α=0.5，β=0.5)")
    ...
    print("Training completed (平衡权重α=0.5，β=0.5)!")  # 修改日志
    print(f"最佳验证Dice (平衡权重α=0.5，β=0.5): {best_dice:.4f}")  # 修改日志
    return best_dice
...
def plot_metrics(history):
    ...
    for i, (title, keys) in enumerate(metrics):
        ...
        plt.title(title)
        plt.xlabel('Epoch (平衡权重α=0.5，β=0.5)')  # 修改标识：X轴
        plt.ylabel(title)
        plt.legend()
        plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("results/loss_2.png", dpi=300, bbox_inches='tight')  # 修改路径
    plt.close()
    print("Metrics plot (平衡权重α=0.5，β=0.5) saved to results/training_metrics_balanced_loss.png")  # 修改日志


if __name__ == "__main__":
    best_dice = train_model()
    print(f"最终最佳Dice系数 (平衡权重α=0.5，β=0.5): {best_dice:.4f}")  # 修改日志