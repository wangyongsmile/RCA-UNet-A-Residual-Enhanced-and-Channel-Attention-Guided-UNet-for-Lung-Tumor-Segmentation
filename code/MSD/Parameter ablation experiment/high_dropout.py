...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # Dropout概率提升至0.6和0.5（原0.4、0.3）
        self.dropout = nn.Dropout2d(0.6)
        self.extra_dropout = nn.Dropout2d(0.5)
...
class RCAUNet(nn.Module):
    # 模型架构（完全不变，仅ResidualBlock的Dropout参数已修改）
...
def train_model():
    ...
    # 初始化模型（ResidualBlock已改为高Dropout）
    model = RCAUNet(in_channels=1, out_channels=1).to(DEVICE)
    total_params = sum(p.numel() for p in model.parameters())
    # 日志标注“高Dropout”
    print(f"模型参数总数（高Dropout: Dropout2d=0.6, extra_dropout=0.5）: {total_params:,}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # 训练进度条标注“高Dropout”
        train_progress = tqdm(train_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Train (高Dropout)]")
        optimizer.zero_grad()
        ...
        with torch.no_grad():
            # 验证进度条标注“高Dropout”
            val_progress = tqdm(val_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Val (高Dropout)]")
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, "results/best_model_high_dropout.pth")  # 标注“高Dropout”
            print(f"New best model (高Dropout: Dropout2d=0.6, extra_dropout=0.5) saved with Dice: {val_dice:.4f}, IoU: {val_iou:.4f}")  # 标注参数
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        if patience_counter >= max_patience:
            print(f"Early stopping triggered after {epoch + 1} epochs (高Dropout)")
        ...
        # 训练日志（标注“高Dropout”）
        print(f"Epoch {epoch + 1}/{NUM_EPOCHS} (高Dropout: Dropout2d=0.6, extra_dropout=0.5)")
    ...
    print("Training completed (高Dropout)!")
    print(f"最佳验证Dice (高Dropout: Dropout2d=0.6, extra_dropout=0.5): {best_dice:.4f}")
    return best_dice
...
def plot_metrics(history):
    ...
    for i, (title, keys) in enumerate(metrics):
        ...
        plt.title(title)
        plt.xlabel('Epoch (高Dropout: Dropout2d=0.6, extra_dropout=0.5)')  # 标注具体参数
        plt.ylabel(title)
        plt.legend()
    ...
    plt.tight_layout()
    # 图表保存路径（标注“高Dropout”）
    plt.savefig("results/high_dropout_2.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Metrics plot (高Dropout) saved to results/training_metrics_high_dropout.png")


if __name__ == "__main__":
    best_dice = train_model()
    print(f"最终最佳Dice系数 (高Dropout: Dropout2d=0.6, extra_dropout=0.5): {best_dice:.4f}")