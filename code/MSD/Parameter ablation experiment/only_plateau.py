...
# 核心修改：删除CosineAnnealingWarmRestarts的导入
...
def train_model():
    ...
    # 日志标注实验名称
    print(f"模型参数总数（仅ReduceLROnPlateau调度器）: {total_params:,}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # 训练进度条标注实验名称
        train_progress = tqdm(train_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Train (仅ReduceLROnPlateau)]")
        optimizer.zero_grad()
        ...
        with torch.no_grad():
            # 验证进度条标注实验名称
            val_progress = tqdm(val_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Val (仅ReduceLROnPlateau)]")
        ...
        # Plateau调度器step（必须在验证后调用，避免val_dice未定义）
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, "results/best_model_only_plateau.pth")  # 标注实验名称
            print(f"New best model (仅ReduceLROnPlateau) saved with Dice: {val_dice:.4f}, IoU: {val_iou:.4f}")  # 标注实验名称
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        if patience_counter >= max_patience:
            print(f"Early stopping triggered after {epoch + 1} epochs (仅ReduceLROnPlateau)")
        ...
        # 训练日志（标注实验名称）
        print(f"Epoch {epoch + 1}/{NUM_EPOCHS} (仅ReduceLROnPlateau)")
    ...
    print("Training completed (仅ReduceLROnPlateau)!")
    print(f"最佳验证Dice (仅ReduceLROnPlateau): {best_dice:.4f}")
    return best_dice
...
def plot_metrics(history):
    ...
    for i, (title, keys) in enumerate(metrics):
        ...
        plt.title(title)
        plt.xlabel('Epoch (仅ReduceLROnPlateau)')  # 标注实验名称
        plt.ylabel(title)
        plt.legend()
        plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("results/only_plateau_2.png", dpi=300, bbox_inches='tight')  # 标注实验名称
    plt.close()
    print("Metrics plot (仅ReduceLROnPlateau) saved to results/training_metrics_only_plateau.png")


if __name__ == "__main__":
    best_dice = train_model()
    print(f"最终最佳Dice系数 (仅ReduceLROnPlateau): {best_dice:.4f}")