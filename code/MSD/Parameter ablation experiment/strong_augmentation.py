...
class OptimizedLungDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_training:
            ...
            # 2. 0-360°随机旋转（替代原90°倍数旋转）
            if random.random() > 0.5:
                h, w = image.shape
                angle = random.uniform(0, 360)  # 0-360度任意角度
                # 计算旋转矩阵（以图像中心为旋转点）
                M = cv2.getRotationMatrix2D((w/2, h/2), angle, 1)
                # 旋转图像（双线性插值）和掩码（最近邻插值）
            ...
            if random.random() > 0.4:
                h, w = image.shape
                scale = random.uniform(0.8, 1.2)  # 强增强：更大的缩放范围
            ...
            # 4. 高斯模糊概率提高到0.5
            if random.random() > 0.5:  # 50%概率执行模糊
                image = cv2.GaussianBlur(image, (3, 3), 0)

            # 5. 亮度调整概率提高到0.7
            if random.random() > 0.3:  # 70%概率执行亮度调整
                brightness_factor = random.uniform(0.85, 1.15)  # 更大的亮度变化范围
                image = image * brightness_factor

            # 6. 对比度调整概率提高到0.7
            if random.random() > 0.3:  # 70%概率执行对比度调整
                contrast_factor = random.uniform(0.85, 1.15)  # 更大的对比度变化范围
                image_mean = np.mean(image)
                image = (image - image_mean) * contrast_factor + image_mean

            # 7. 中等噪声添加概率提高到0.6
            if random.random() > 0.4:  # 60%概率添加噪声
                noise_std = random.uniform(0, 0.02)  # 更强的噪声
...
def train_model():
    ...
    print(f"模型参数总数 (强数据增强): {total_params:,}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, "results/best_model_strong_augmentation.pth")
            print(f"New best model (强数据增强) saved with Dice: {val_dice:.4f}, IoU: {val_iou:.4f}")
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        # 日志添加强数据增强标识
        print(f"Epoch {epoch + 1}/{NUM_EPOCHS} (强数据增强)")
    ...
    print("Training completed (强数据增强)!")
    print(f"最佳验证Dice (强数据增强): {best_dice:.4f}")
    return best_dice
...
def plot_metrics(history):
    ...
    for i, (title, keys) in enumerate(metrics):
        ...
        plt.title(title)
        plt.xlabel('Epoch (强数据增强)')  # 添加强数据增强标识
        plt.ylabel(title)
        plt.legend()
    ...
    plt.tight_layout()
    # 保存路径为强数据增强标识
    plt.savefig("results/strong_augmentation.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Metrics plot (强数据增强) saved to results/training_metrics_strong_augmentation.png")


if __name__ == "__main__":
    best_dice = train_model()
    print(f"最终最佳Dice系数 (强数据增强): {best_dice:.4f}")
