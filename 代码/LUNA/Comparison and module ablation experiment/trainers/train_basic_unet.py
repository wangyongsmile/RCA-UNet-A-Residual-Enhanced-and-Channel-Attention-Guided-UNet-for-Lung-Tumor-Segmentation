...
def train_basic_unet():
    ...
    model = BasicUNet(in_channels=1, out_channels=1).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))

    return train_model(model, train_dataset, val_dataset, test_dataset, "BasicUNet", "basic_unet")