...
from models import IncrementalMRRN
from trainers.base_trainer import train_model


def train_incremental_mrrn():
    ...
    model = IncrementalMRRN(in_channels=1, out_channels=1, num_streams=3, base_channels=32).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))

    return train_model(model, train_dataset, val_dataset, test_dataset, "Incremental-MRRN", "incremental_mrrn")