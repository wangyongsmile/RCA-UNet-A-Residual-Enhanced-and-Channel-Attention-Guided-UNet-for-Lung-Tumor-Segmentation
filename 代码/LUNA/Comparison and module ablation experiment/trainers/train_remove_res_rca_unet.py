...
from models import RCAUNetWithoutRes
from trainers.base_trainer import train_model


def train_remove_res_rca_unet():
    ...
    model = RCAUNetWithoutRes(in_channels=1, out_channels=1).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))

    return train_model(model, train_dataset, val_dataset, test_dataset, "RCAUNet-无残差", "rca_unet_no_res")