...
# ========== 平衡权重 ==========
class BalancedFocalDiceLoss(nn.Module):
    def __init__(self, alpha=0.8, gamma=2.0, dice_weight=0.5, focal_weight=0.3, boundary_weight=0.2, smooth=1e-6):
        super(BalancedFocalDiceLoss, self).__init__()
        self.alpha = alpha
        self.gamma = gamma
...
def train_model():
    ...
    # ========== 使用平衡权重损失函数 ==========
    criterion = BalancedFocalDiceLoss(
        alpha=0.8, gamma=2.0,
        dice_weight=0.5, focal_weight=0.3, boundary_weight=0.2
    )
    print("使用损失函数: 平衡权重 (dice_weight=0.5, focal_weight=0.3, boundary_weight=0.2)")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, os.path.join(BASE_PATH, "trained_models", "best_rca_unet_balanced.pth"))
    ...
    # 测试最佳模型
    test_model(model, test_loader, DEVICE, os.path.join(BASE_PATH, "trained_models", "best_rca_unet_balanced.pth"))
...
def plot_metrics(history):
    ...
    plt.tight_layout()
    plt.savefig("results/training_metrics_balanced_weights.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Metrics plot saved to results/training_metrics_balanced_weights.png")
