...
# ========== 无增强 ==========
class NoAugmentationPreprocessor:
    def __init__(self, target_size=(256, 256)):
        self.target_size = target_size

        # 仅保留Resize和归一化，删除所有数据增强
...
def train_model():
    ...
    preprocessor = NoAugmentationPreprocessor(target_size=(256, 256))  # 使用无增强预处理器
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, os.path.join(BASE_PATH, "trained_models", "best_rca_unet_no_aug.pth"))
    ...
    # 测试最佳模型
    test_model(model, test_loader, DEVICE, os.path.join(BASE_PATH, "trained_models", "best_rca_unet_no_aug.pth"))
...
def plot_metrics(history):
    ...
    plt.tight_layout()
    plt.savefig("results/training_metrics_no_augmentation.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Metrics plot saved to results/training_metrics_no_augmentation.png")
