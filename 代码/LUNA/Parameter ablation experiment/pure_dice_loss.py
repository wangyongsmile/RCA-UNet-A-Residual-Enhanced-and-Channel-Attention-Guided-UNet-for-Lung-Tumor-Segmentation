...
# ========== 纯 Dice Loss ==========
class PureDiceLoss(nn.Module):
    def __init__(self, smooth=1e-6):
        super(PureDiceLoss, self).__init__()
        self.smooth = smooth
...
def train_model():
    ...
    # ========== 使用纯 Dice Loss ==========
    criterion = PureDiceLoss()
    print("使用损失函数: 纯Dice Loss (dice_weight=1.0, focal_weight=0.0, boundary_weight=0.0)")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, os.path.join(BASE_PATH, "trained_models", "best_rca_unet_pure_dice.pth"))
    ...
    # 测试最佳模型
    test_model(model, test_loader, DEVICE, os.path.join(BASE_PATH, "trained_models", "best_rca_unet_pure_dice.pth"))
...
def plot_metrics(history):
    ...
    plt.tight_layout()
    plt.savefig("results/training_metrics_pure_dice.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Metrics plot saved to results/training_metrics_pure_dice.png")
