...
def train_model():
    ...
    # 初始化模型（ResidualBlock已无Dropout）
    model = RCAUNet(in_channels=1, out_channels=1).to(DEVICE)
    total_params = sum(p.numel() for p in model.parameters())
    # 日志标注“无Dropout”
    print(f"模型参数总数（无Dropout）: {total_params:,}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # 训练进度条标注“无Dropout”
        train_progress = tqdm(train_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Train (无Dropout)]")
        optimizer.zero_grad()
        ...
        with torch.no_grad():
            # 验证进度条标注“无Dropout”
            val_progress = tqdm(val_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Val (无Dropout)]")
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, "results/best_model_no_dropout.pth")  # 标注“无Dropout”
            print(f"New best model (无Dropout) saved with Dice: {val_dice:.4f}, IoU: {val_iou:.4f}")  # 标注“无Dropout”
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        if patience_counter >= max_patience:
            print(f"Early stopping triggered after {epoch + 1} epochs (无Dropout)")
        ...
        # 训练日志（标注“无Dropout”）
        print(f"Epoch {epoch + 1}/{NUM_EPOCHS} (无Dropout)")
    ...
    print("Training completed (无Dropout)!")
    print(f"最佳验证Dice (无Dropout): {best_dice:.4f}")
    return best_dice
...
def plot_metrics(history):
    ...
    for i, (title, keys) in enumerate(metrics):
        ...
        plt.title(title)
        plt.xlabel('Epoch (无Dropout)')  # 标注“无Dropout”
        plt.ylabel(title)
        plt.legend()
    ...
    plt.tight_layout()
    # 图表保存路径（标注“无Dropout”）
    plt.savefig("results/no_dropout_1.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Metrics plot (无Dropout) saved to results/training_metrics_no_dropout.png")


if __name__ == "__main__":
    best_dice = train_model()
    print(f"最终最佳Dice系数 (无Dropout): {best_dice:.4f}")