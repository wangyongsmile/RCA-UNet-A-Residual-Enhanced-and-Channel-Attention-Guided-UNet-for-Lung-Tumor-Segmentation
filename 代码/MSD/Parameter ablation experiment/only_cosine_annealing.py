...
def train_model():
    ...
    # 日志标注“仅Cosine Annealing”
    print(f"模型参数总数（仅Cosine Annealing调度器）: {total_params:,}")
    ...
    # 仅保留Cosine Annealing调度器（参数符合要求：T0=20，eta_min=1e-7）
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # 训练进度条标注“仅Cosine Annealing”
        train_progress = tqdm(train_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Train (仅Cosine Annealing)]")
        optimizer.zero_grad()
        ...
        # 仅执行Cosine Annealing调度器步进（删除Plateau的step）
        ...
        with torch.no_grad():
            # 验证进度条标注“仅Cosine Annealing”
            val_progress = tqdm(val_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Val (仅Cosine Annealing)]")
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, "results/best_model_only_cosine_annealing.pth")  # 标注仅Cosine路径
            print(f"New best model (仅Cosine Annealing) saved with Dice: {val_dice:.4f}, IoU: {val_iou:.4f}")  # 日志标识
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        if patience_counter >= max_patience:
            print(f"Early stopping triggered after {epoch + 1} epochs (仅Cosine Annealing)")
        ...
        # 训练日志（修改标识）
        print(f"Epoch {epoch + 1}/{NUM_EPOCHS} (仅Cosine Annealing)")
    ...
    print("Training completed (仅Cosine Annealing)!")  # 日志标识
    print(f"最佳验证Dice (仅Cosine Annealing): {best_dice:.4f}")  # 日志标识
    return best_dice
...
def plot_metrics(history):
    ...
    for i, (title, keys) in enumerate(metrics):
        ...
        plt.title(title)
        plt.xlabel('Epoch (仅Cosine Annealing)')  # 核心修改：X轴标注仅Cosine
        plt.ylabel(title)
        plt.legend()
        plt.grid(True, alpha=0.3)
    plt.tight_layout()
    # 核心修改：图表保存路径（标注仅Cosine）
    plt.savefig("results/tonly_cosine_annealing.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Metrics plot (仅Cosine Annealing) saved to results/training_metrics_only_cosine_annealing.png")  # 日志标识


if __name__ == "__main__":
    best_dice = train_model()
    print(f"最终最佳Dice系数 (仅Cosine Annealing): {best_dice:.4f}")  # 日志标识