...
class OptimizedCombinedLoss(nn.Module):
    def __init__(self, alpha=1.0, beta=0.0):  # 纯Dice Loss权重
        super(OptimizedCombinedLoss, self).__init__()
        self.alpha = alpha  # Dice损失权重=1.0
        self.beta = beta    # BCE损失权重=0.0（无贡献）
    ...
    def forward(self, inputs, targets):
        ...
        total_loss = self.alpha * dice_loss + self.beta * bce_loss  # 实际=纯Dice Loss
        return total_loss
...
def train_model():
    ...
    print(f"模型参数总数（纯Dice Loss）: {total_params:,}")  # 修改标识

    # 显式初始化纯Dice Loss
    criterion = OptimizedCombinedLoss(alpha=1.0, beta=0.0)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        train_progress = tqdm(train_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Train (纯Dice Loss)]")  # 修改标识
        ...
        with torch.no_grad():
            val_progress = tqdm(val_loader, desc=f"Epoch {epoch + 1}/{NUM_EPOCHS} [Val (纯Dice Loss)]")  # 修改标识
        ...
        if val_dice > best_dice:
            ...
            torch.save({
                ...
                'best_dice': best_dice,
            }, "results/best_model_pure_dice_loss.pth")  # 修改路径
            print(f"New best model (纯Dice Loss) saved with Dice: {val_dice:.4f}, IoU: {val_iou:.4f}")  # 修改标识
            patience_counter = 0
        else:
            patience_counter += 1

        if patience_counter >= max_patience:
            print(f"Early stopping triggered after {epoch + 1} epochs (纯Dice Loss)")  # 修改标识
        ...
        # 修改日志标识
        print(f"Epoch {epoch + 1}/{NUM_EPOCHS} (纯Dice Loss)")
    ...
    print("Training completed (纯Dice Loss)!")  # 修改标识
    print(f"最佳验证Dice (纯Dice Loss): {best_dice:.4f}")  # 修改标识
    return best_dice
...
def plot_metrics(history):
    ...
    for i, (title, keys) in enumerate(metrics):
        ...
        plt.title(title)
        plt.xlabel('Epoch (纯Dice Loss)')  # 修改标识
        plt.ylabel(title)
        plt.legend()
        plt.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig("results/pure_dice_loss.png", dpi=300, bbox_inches='tight')  # 修改路径
    plt.close()
    print("Metrics plot (纯Dice Loss) saved to results/training_metrics_pure_dice_loss.png")  # 修改标识


if __name__ == "__main__":
    best_dice = train_model()
    print(f"最终最佳Dice系数 (纯Dice Loss): {best_dice:.4f}")  # 修改标识
